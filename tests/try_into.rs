...
#[derive(Debug, Clone, Copy, TryInto, PartialEq, Eq)]
...
fn test_try_into() {
    ...
    assert_eq!(
        i64::try_from(i),
        Err(("Only NamedBigInt, UnsignedWithIgnoredField, NamedUnsignedWithIgnnoredField can be converted to i64", i))
    );
    assert_eq!(
        <(i32, i32)>::try_from(i),
        Err(("Only TwoSmallInts can be converted to (i32, i32)", i))
    );
    assert_eq!(
        <(i64, i64)>::try_from(i),
        Err(("Only NamedBigInts can be converted to (i64, i64)", i))
    );
    assert_eq!(
        u32::try_from(i),
        Err(("Only Unsigned, NamedUnsigned can be converted to u32", i))
    );
    assert_eq!(
        <()>::try_from(i),
        Err(("Only Unit can be converted to ()", i))
    );

    let mut i = MixedInts::NamedBigInt { int: 42 };
    assert_eq!(
        i32::try_from(i),
        Err(("Only SmallInt can be converted to i32", i))
    );
